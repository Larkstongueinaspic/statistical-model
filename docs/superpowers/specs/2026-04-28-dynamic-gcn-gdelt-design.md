...
- 方向：默认双向合并。只要事件满足 `(Actor1CountryCode == exporter_iso3 and Actor2CountryCode == "CHN")` 或 `(Actor2CountryCode == exporter_iso3 and Actor1CountryCode == "CHN")`，就归入该 `exporter_code, year`。压力指标只关心该国家对与中国之间的摩擦强度，不解释为单向施压。
...
- 默认 `gdelt_apply_keyword_filter=False`，即认为输入 CSV 已经预筛选，模块只聚合。
- 若显式开启 `gdelt_apply_keyword_filter=True`，必须存在 `SOURCEURL`、`DocumentIdentifier` 或 `EventText` 中至少一个字段。
- 关键词表固定为：`semiconductor`, `chip`, `chips`, `integrated circuit`, `export control`, `sanction`, `technology restriction`, `trade restriction`, `entity list`, `supply chain`。
- 过滤规则为大小写不敏感的子串匹配；任一关键词命中即保留。
- 输出 `gdelt_filter_mode` 字段，取值为 `pre_filtered`, `keyword_filtered`, `disabled_gdelt`。
...
gdelt_avg_tone, gdelt_pressure_score, gdelt_filter_mode
```
...
- 核心 4 产品 MAE，作为 `sample_scope=core4_model_products` 的单独指标行输出
...
`graph_level_features_v03.csv` schema：

```text
sample_id, product_code, product_group, is_core_product,
graph_year, target_year, split,
current_siri_score, target_siri,
concentration_raw, policy_exposure_raw,
alternative_insufficiency_raw, structural_volatility_raw,
log_total_import_value, source_count, top1_import_share,
usa_import_share, weighted_gdelt_pressure_score,
...
- naive、Ridge、GCN 必须在同一批 `status=usable` 的 `sample_id` 上比较。
- `current_siri_score` 缺失时，该样本对所有模型同时剔除，并记录 `missing_current_siri`。
- Ridge 特征缺失值统一填 0，并在验证表统计填充次数。
- 若某核心 4 产品只是 `report_only` 而没有进入 `model`，它不进入 MAE 计算；报告表单独列出 `not_in_model_reason`。
...
```text
model, split, sample_scope, mae, rmse, spearman_rank_corr,
...
- `all_model_products`
- `core4_model_products`

核心 4 产品指标通过 `sample_scope=core4_model_products` 的行表达；若该 scope 没有样本，指标为空，`n_samples=0`。
...
| `trade_graphs.py` | panel, targets, gdelt pressure | `GraphSample` list, `graph_samples_v03.csv`, `graph_level_features_v03.csv` | 构造年度产品图、节点特征和图级特征 |
| `baselines.py` | `graph_level_features_v03.csv` | baseline predictions, metrics | 训练 naive 和 Ridge 基线 |
...
| `disable_gdelt` | `False` |
| `allow_baci_only` | `False`，必须由 CLI 显式开启 |
| `gdelt_apply_keyword_filter` | `False` |
...
- 产品年份覆盖不足：写入剔除原因。
- GDELT 文件缺失：只有当 `disable_gdelt=True` 且 `allow_baci_only=True` 时继续 BACI-only；否则停止并提示准备 GDELT 文件或显式开启 BACI-only。
- 若 `disable_gdelt=True` 但 `allow_baci_only=False`，停止并提示用户确认该降级模式，避免误把无 GDELT 结果写成 GDELT 结果。
